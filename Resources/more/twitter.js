...
var twitter_name = 'PoliticalFiber';
var win = Ti.UI.currentWindow;
win.title = '@'+twitter_name;
var feed = "http://api.twitter.com/1/statuses/user_timeline.json?screen_name="+twitter_name;
		...
		try
		{
			...
			for (var c=0;c<tweets.length;c++){
				...
				if (Titanium.Platform.name == 'android') {
					var row = Ti.UI.createTableViewRow({hasChild:true,height:100,backgroundColor:bgcolor});
				} else {
				...
				row.addEventListener('click',function(e){
           			var w = Ti.UI.createWindow({title:screen_name});
            		var wb = Ti.UI.createWebView({url:"http://twitter.com/#!/"+screen_name});
            		w.add(wb);
